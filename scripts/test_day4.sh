...
timeout --signal=SIGINT --kill-after=2s 20s \
  ros2 run robot_dog_day4_actions gait_action_server >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
...
set +e
valid_output="$(ros2 run robot_dog_day4_actions gait_action_client \
  --ros-args \
  -p gait_name:=TROT \
  -p step_count:=3 \
...
valid_status=$?
invalid_output="$(ros2 run robot_dog_day4_actions gait_action_client \
  --ros-args \
  -p gait_name:=FLY \
  -p step_count:=3 \
  -p step_period_ms:=100 \
